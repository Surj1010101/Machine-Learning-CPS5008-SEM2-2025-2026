...
def print_ethical_notes():
    ...
    This is the ethics reflection content, the aim is to keep the notes
    in one place so the report can quote them verbatim and the file output can be saved
    as evidence. This also returns the same text so run.py can write it to the
    outputs folder.
    """
    print("\n" + "=" * 70)
...
1. AUTOMATION BIAS
   ...
   review. The main aim here is that automation should support the reviewer
...
2. PRIVACY
   ...
   features. This also means in production that model training on
...
3. FEEDBACK LOOPS
   ...
   pattern. This is the classic feedback loop risk in operational ML,
...
4. SYNTHETIC DATA CAVEAT
   ...
   methodology, not about any real customer population. the aim is
...
5. FAIRNESS LIMITS
   ...
   the dataset. This also means that my fairness conclusions are
...
7. EXPLAINABILITY AND ACCOUNTABILITY
   ...
   top so individual predictions can be defended. This also matters for
